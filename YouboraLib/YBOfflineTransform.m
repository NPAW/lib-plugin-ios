...
- (void) saveEventWithParams:(NSMutableDictionary*) params andService:(NSString *) service{
    ...
    params[@"unixtime"] = [NSString stringWithFormat:@"%.0f", round(CFAbsoluteTimeGetCurrent()*1000)];
    ...
    [self.dataSource lastIdWithCompletion:^(NSNumber* offlineId){
        ...
        [self.dataSource allEventsWithCompletion:^(NSArray * events) {
            ...
            if(jsonEvents != nil){
                ...
                [self.dataSource putNewEvent:event completion:^(NSString* eventId){
                    [YBLog debug: @"Inserted event with id: %@",eventId];
                }];
            }
